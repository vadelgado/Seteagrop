...
from application.auth.formulario.formularios import RegistrationForm, LoginForm, EditForm
from application import login_manager_app, mail
...
def send_confirmation_email(user):
    """
    Método para el envío de correos con archivo HTML adjunto.
    """
    subject = "Confirmación de registro"
    
    # Renderizar el contenido del archivo HTML usando Flask's render_template
    body = render_template('confirmacion_registro.html', user=user)

    message = Message(subject, recipients=[user.correo], body=body, sender='pruebaseteagrop@gmail.com', html=body)
    mail.send(message)
...
@auth.route("/auth/edit_user", methods=["GET", "POST"])
@login_required
def edit_user():
    """
