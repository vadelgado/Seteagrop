...
class ModelUser():
    ...
    def update_user(cls, db, user):
        ...
        Argumentos:
            db (objeto): Conexión a la base de datos.
            user (User): Objeto de usuario con la información actualizada.
        Excepciones:
            ValueError: Si se produce un error durante la operación.
        """
        try:
            cursor = db.connection.cursor()
            sql = "UPDATE usuarios SET nombre = %s, apellido = %s, tipo_identificacion = %s, identificacion = %s WHERE correo = %s"
            cursor.execute(sql, (user.nombre, user.apellido, user.tipo_identificacion, user.identificacion, user.correo))
            db.connection.commit()
            cursor.close()
        except Exception as ex:
            db.connection.rollback()  # Revertir la transacción en caso de error
